...
@interface ZJRetweetViewController ()<UITextViewDelegate,ZJComposeToolBarDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate>
...
@implementation ZJRetweetViewController
...
{
    /*
     https://api.weibo.com/2/statuses/repost.json post
    ...
    //2.发送请求
    [ZJHttpTool post:@"https://api.weibo.com/2/statuses/repost.json" params:params success:^(id json) {
        //                ZJLog(@"请求成功--%@",json[@"text"]);
        [MBProgressHUD showSuccess:@"转发成功!"];
    } failure:^(NSError *error) {
        //                ZJLog(@"请求失败--%@",error);
        [MBProgressHUD showError:@"网络繁忙，麻烦重新转发!"];
